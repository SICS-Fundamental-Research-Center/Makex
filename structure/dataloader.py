...
class RuleDataset(Dataset):
	def __init__(self, num_encode, rule_list=None):
		self.rules = list()
		self.num_encode = num_encode
		self.ending_idx = num_encode +1
		self.padding_idx = num_encode + 2
		if rule_list != None:
			for rule in rule_list:
				rule_len = rule[0]
				formatted_rule = [[rule[1 + k] for k in range(rule_len)] + [self.ending_idx], self.padding_idx, rule[-3] + 1e-5]
